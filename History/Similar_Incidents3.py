...
def main():
    """
    Enhanced main function to run the incident analysis system with better error handling,
    user interaction, and session management.
    """
    def print_banner():
        print("\n" + "="*50)
        print("Enhanced Incident Analysis System")
        print("="*50 + "\n")


    def get_historical_file():
        """Get historical incidents file path with validation."""
        while True:
            default_path = r'C:\Users\gurhegde\OneDrive - Deloitte (O365D)\CAI Playground\Pratik-Tasks\Unilever POC\Integration 1.2\History\Incidents_4X3.xlsx'
            print("\nHistorical incidents file:")
            print(f"1. Use default path: {default_path}")
            ...
            if choice == "1":
                if os.path.exists(default_path):
                    return default_path
                print("Error: Default file not found.")
            elif choice == "2":
                custom_path = input("Enter the full path to historical incidents file: ").strip()
    ...
    def handle_analysis_session(system):
        """Handle a single analysis session with error recovery."""
        while True:
            try:
                incident = system.get_incident_input()
                if not incident.strip():
                    print("Error: Empty incident description. Please try again.")
                    continue
                ...
                if result['similar_incidents']:
                    ...
                    for i, similar in enumerate(result['similar_incidents'], 1):
                        ...
                        print("-" * 10)
                        print(f"ID: {similar['incident_id']}")
                        print(f"Similarity: {similar['similarity_score']:.1f}%")
                        print("\nDescription:")
                        print(similar['description'])
                        print("\nActions Taken:")
                        print(similar['actions_taken'])
                        print("\nParticipants:")
                        print(similar['participants'])
                else:
                    print("\nNo similar incidents found in the database.")
    ...
    try:
        ...
        # Initialize system
        api_key = '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'
        historical_file = get_historical_file()
        
        print("\nInitializing system...")
        system = EnhancedIncidentAnalysisSystem(api_key)
        
        # Reset and initialize database
        print("Resetting database...")
        if not system.reset_database():
            raise RuntimeError("Failed to reset database")
        
        print("Loading historical incidents...")
        system.load_historical_incidents(historical_file)
        handle_analysis_session(system)
        

        
        print("\nThank you for using the Enhanced Incident Analysis System!")
        
    except Exception as e:
        logger.error(f"Fatal error: {str(e)}")
        print("\nERROR: The system encountered a fatal error and must exit.")
